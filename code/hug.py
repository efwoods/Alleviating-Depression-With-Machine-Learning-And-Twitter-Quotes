...
for text in range(0,testdf["text"].size):
    ...
    if testdf["sentiment"][text] == "Negative":
        ...
        if not tweetid in file_contents:
            ...
            f.close()
            try:
                api.update_status(status = '*gives hug* Everyone has something to give. Find what you love most, and grow towards mastery in what you love.', in_reply_to_status_id = tweetid , auto_populate_reply_metadata=True)
            except Exception:
                pass